...
2. Global blocks content:
   ...
   - The global instructions must also mirror the Ralph loop commit contract exactly:
     - the executor must not run `git commit` or `git push` directly
     - `.loop-commit-msg` must contain exactly one non-empty line
     - the only valid commit subject formats are `{phase}-{plan}-task{task-number}`, `{phase}-{plan}-test{test-number}`, and `{phase}-{plan}-summary`
     - implementation sections generated by this command must use only the implementation-relevant formats: `{phase}-{plan}-task{task-number}` for task sections and `{phase}-{plan}-summary` for summary sections
...
4. Task execution sections:
   ...
   - Steps must include:
     ...
     9. Write `.loop-commit-msg` with exactly one non-empty line: `<plan-id>-task<task-number>`.
...
5. Summary section:
   ...
   - Steps must include:
     ...
     5. Write `.loop-commit-msg` with exactly one non-empty line: `<plan-id>-summary`.
...
6. Summary creation instructions (used any time a section references `SUMMARY.md`):
   ...
   - Ralph loop commit subject formats are:
     - implementation task commits: `{phase}-{plan}-task{task-number}`
     - testing commits: `{phase}-{plan}-test{test-number}`
     - summary/metadata commits: `{phase}-{plan}-summary`
   - For summaries generated from this implementation plan, use only the implementation task commits and the matching summary commit as evidence for what was built. Do not treat `{phase}-{plan}-test*` commits as implementation evidence.
   - For plan `01-02`, identify relevant commits with:
     - `git log --oneline --grep="^01-02-task[0-9]+$"`
     - `git log --oneline --grep="^01-02-summary$"`
   - Treat `01-02-task*` matches as implementation task commits, ignore any `01-02-test*` commits for this summary, and treat `01-02-summary` as the summary/metadata commit.
   - Use `git show --stat --name-status <commit>` across the implementation task commits to determine which files were created or modified and to support any summary of what was added.
