...
6. Summary creation instructions (used any time a section references `SUMMARY.md`):
   ...
   - Implementation-plan commit subject formats are:
     - implementation task commits: `{phase}-{plan}-task{task-number}`
     - summary/metadata commits: `{phase}-{plan}-summary`
   - For summaries generated from this implementation plan, use the implementation task commits as the evidence for what was built.
   - For plan `01-02`, identify relevant commits with:
     - `git log --oneline --grep="^01-02-task[0-9]+$"`
   - Treat `01-02-task*` matches as the implementation task commits for that plan.
